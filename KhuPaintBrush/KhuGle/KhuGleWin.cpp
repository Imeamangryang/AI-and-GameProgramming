...
void PlaceInCenterOfScreen(HWND hDlg)
{
	RECT rect;
...
LRESULT CALLBACK CKhuGleWin::WndProcInstanceMember(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_COMMAND:
		...
		{
		case IDOK:
			GetDlgItemText(hwnd, IDC_EDIT1, textbuf, 128);
			GetDlgItemText(hwnd, IDC_EDIT2, textbuf2, 128);
			EndDialog(hwnd, TRUE);
			m_pWinApplication->ButtonCheck = false;
			m_pWinApplication->ButtonCheck2 = false;
			return true;
	...
	int       nCmdShow)
{
	...
	while(1)
	{
		...
		else
		{
			...
			if (CKhuGleWin::m_pWinApplication->ButtonCheck == true)
			{
				DialogBox(windowClass.hInstance, MAKEINTRESOURCE(IDD_DIALOG1), NULL, CKhuGleWin::WndProc);
			}
			if (CKhuGleWin::m_pWinApplication->ButtonCheck2 == true)
			{
				DialogBox(windowClass.hInstance, MAKEINTRESOURCE(IDD_DIALOG2), NULL, CKhuGleWin::WndProc);
			}
		}
	}
